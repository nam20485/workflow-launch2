...
* **Epic: Implement OdbDesignGrpcServiceImpl**  
  ...
  * **User Story 2:** As a Developer, I want to implement the GetAvailableDesigns and GetStepList unary RPC methods.  
    * **Tasks:**  
      ...
      * $$ $$  
        Add nullptr checks and try...catch blocks as defined in US 4\.  
  ...
  * **User Story 5:** As a Developer, I want all gRPC service methods to be robust against C++ exceptions.  
    * **Tasks:**  
      * $$ $$  
        Wrap the entire body of *all* overridden service methods (GetLayerList, GetSymbols, GetLayerFeaturesStream, etc.) in a try...catch block.  
        try {  
            // ... (all implementation logic from User Stories 2 & 3\) ...  
      ...
      * This is critical for server stability; an uncaught exception in an RPC handler can crash the entire server process.  
  * **User Story 6: Implement gRPC Authentication**  
    * **Tasks:**  
      ...
      * $$ $$  
        **Refactor:** Create a new shared utility (e.g., in Utils/AuthUtils.h/.cpp).  
      * $$ $$  
        Create a function: std::pair\<std::string, std::string\> ParseBasicAuthHeader(std::string\_view auth\_header). This function will handle stripping the 'Basic ' prefix and Base64-decoding the token.  
      * $$ $$  
        Move the user/pass validation logic (from BasicRequestAuthentication::before\_handle) into a shared function: bool ValidateCredentials(const std::string& user, const std::string& pass).  
      * $$ $$  
        The Crow middleware will call these functions using the authorization header from the crow::request.  
      * $$ $$  
        The gRPC AuthInterceptor will call these functions using the authorization metadata value from context-\>client\_metadata(). This ensures 100% logic re-use.  
      * $$ $$  
...
* **Epic: Update OdbDesignServer/main.cpp**  
  * **User Story 1:** As a Developer, I want to instantiate and run the new gRPC server in a background thread.  
    * **Tasks:**  
      * $$ $$  
        Include \<thread\>, \<atomic\>, \<csignal\>, and the new OdbDesignGrpcServiceImpl.h (and AuthInterceptor.h) in main.cpp.  
      * $$ $$  
      ...
      * $$ $$  
        Inside this function, instantiate the service: OdbDesignGrpcServiceImpl service(pDesignCache);.  
      * $$ $$  
      ...
      * $$ $$  
        In main(), set up signal handling and a std::atomic\<bool\> shutdown\_flag(false).  
      * $$ $$  
        Launch the gRPC thread:  
        std::thread grpc\_thread(RunGrpcServer, app.getDesignCache(), args, std::ref(shutdown\_flag));

      * $$ $$  
        Run the Crow app (blocking).  
      * $$ $$  
        After app.Run() returns (e.g., on SIGTERM), set shutdown\_flag \= true; and call grpc\_thread.join(); to ensure a clean shutdown of both servers.
...
* **Tasks:**  
  ...
  * $$ $$  
    ...
    * $$ $$  
      ...
      * REST: Time-to-Last-Byte for .../features (JSON).  
    ...
    * $$ $$  
      **Success Criteria:** gRPC TTI \< 25% of REST TTI. gRPC TTFF \< 100ms. Server memory usage per-request is measurably lower due to no large JSON string allocation. **gRPC total payload size \< 10% of REST JSON payload size.**
...
* **Tasks:**  
  ...
  * $$ $$  
    ...
    * $$ $$  
      Implement the standard grpc.health.v1.Health service in the gRPC server.  
    * $$ $$  
...
| **Authentication Logic** | Medium | High | Sharing stateful authentication logic between Crow (HTTP 1.1) and gRPC (HTTP/2) can be difficult. **Mitigation:** **Refactor** the core validation logic (bool IsValid(user, pass)) from BasicRequestAuthentication into a shared, stateless utility function. Both the Crow middleware and the new gRPC AuthInterceptor will call this common function, ensuring a single source of truth for auth logic. |
| **Streaming Performance Overhead** | Low | High | The to\_protobuf() call *inside* the streaming loop still performs serialization, which could be a bottleneck for millions of features. **Mitigation:** The plan already specifies reusing the protoFeature message and calling .Clear() to avoid heap allocations in the loop. This is the 80/20 solution. **Advanced Mitigation (Phase 2):** If profiling shows this is still too slow, the C++ models (FeatureRecord) can be refactored to implement a custom SerializeToZeroCopyStream(grpc::ServerWriter\* writer) method, writing directly to the gRPC buffer and bypassing the intermediate Protobuf object entirely. This is complex but offers maximum performance. |
